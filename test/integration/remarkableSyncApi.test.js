...
describe('reMarkable Sync API integration specs', () => {
	setupHttpRecording()
	...
	it('loads reMarkable root folder', async () => {
		const root = await RemarkableApi.Internal.Sync.Root.fromSession(session)
	...
	afterAll(async () => {
		if(!!folder) { await folder.moveToTrash(session) }
		if(!!pdfFile) { await pdfFile.moveToTrash(session) }
		if(!!epubFile) { await epubFile.moveToTrash(session) }
	})
})
