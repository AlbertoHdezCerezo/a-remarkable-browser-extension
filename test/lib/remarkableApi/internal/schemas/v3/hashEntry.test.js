...
import * as V3 from '../../../../../../src/lib/remarkableApi/internal/schemas/v3'
...
describe('HashEntry', () => {
	describe('.construct', () => {
		it('extracts content from hash entry payload', () => {
			...
			const hashEntry = new V3.HashEntry(hashEntryPayload)
		...
		it('if payload does not resemble a v4 hash entry, throws an error', () => {
			...
			try {
				new V3.HashEntry(hashEntryPayload)
			} catch (error) {
				expect(error).toBeInstanceOf(V3.IncompatibleHashEntrySchemaError)
			}
		})
	})
	...
	describe('#content', () => {
		...
		it('if hash entry content cannot be fetched, throws an error', async () => {
			...
			try {
				...
				expect(error).toBeInstanceOf(V3.UnreachableHashEntryContentError)
			}
		})
	})
})