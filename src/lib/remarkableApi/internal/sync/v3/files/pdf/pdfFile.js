...
class PdfIncompatibleHashEntriesError extends Error {}
...
export default class PdfFile {
	/**
	 * Fetches PDF hash entries from PDF file hash
	 * entry and returns a PdfFile instance.
	 *
	...
	async #updatePdfFileHashEntries(newPdfHashEntries, session) {
		...
		const newPdfFileHashEntriesChecksum = await newPdfFileHashEntriesRequestBuffer.hash()
	...
	async #updateRootHashEntries(newRootHashEntries, session) {
		...
		const newRootHashChecksum = await newRootHashRequestBuffer.hash()
